/* Copyright (C) 2019 Hikaru Sugiura (WHILL,Inc) <whill.jp>
 * 
 ...
 *
 * Utility to send the reset sequence on RTS and DTR suitable for 
 * DTR/RTS XOR Reset Circuit. 
 * 
 * Based on Arduino_STM32 by Roger Clark
 */
...
int setReset(bool state){
	return setRTS(state);
}

int setBoot0(bool state){
	return setDTR(!state);
}
